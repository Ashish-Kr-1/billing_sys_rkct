...
// Load env relative to backend root (parent of scripts dir)
dotenv.config({ path: path.join(__dirname, '../.env') });

(async () => {
    console.log('🔄 Adding is_deleted column to parties table in all databases...');
    ...
    for (const [companyId, config] of Object.entries(DB_CONFIGS)) {
        ...
        try {
            ...
            const [columns] = await connection.query(`
                ...
                WHERE TABLE_SCHEMA = ? AND TABLE_NAME = 'parties' AND COLUMN_NAME = 'is_deleted'
            `, [config.database]);
