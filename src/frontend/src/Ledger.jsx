...
export default function App() {
  ...
  const handleDeletePayment = (transactionId) => {
    setPaymentToDelete(transactionId);
  ...
  const executeDeletePayment = async () => {
    ...
    try {
      const response = await api.delete(`/ledger/payment/${paymentToDelete}`);
      if (response.ok) {
        notify("Payment deleted successfully", "success");
        // Refresh popup history
        const payments = await fetchPaymentHistory(invoicePopup.invoice);
        const paymentRows = payments.map(p => ({
          id: p.id,
          date: new Date(p.date).toISOString().split("T")[0],
        ...
        notify(data.error || "Failed to delete payment", "error");
      }
    } catch (err) {
      console.error(err);
  ...
  const handleRevertInvoice = async (invoice_no) => {
    try {
      const response = await api.put(`/ledger/revert?invoice_no=${encodeURIComponent(invoice_no)}`);
      if (response.ok) {
        notify(`Invoice ${invoice_no} reverted successfully.`, "success");
        fetchLedgerData();
      } else {
        const data = await response.json();
        notify(data.error || 'Failed to revert invoice', "error");
      }
    } catch (error) {
      console.error('Revert invoice error:', error);
      notify('Failed to revert invoice', "error");
    }
  };
                      ...
                      <tr key={index} className={`hover:bg-slate-50 ${isCancelled ? 'bg-red-50/30' : ''}`}>
                        ...
                        <td className="text-center px-4">
                          <div className="flex items-center justify-center gap-2">
                            ...
                            {isCancelled && (
                              <button
                                onClick={() => handleRevertInvoice(row.invoice)}
                                className="text-slate-400 hover:text-emerald-600 transition-colors"
                                title="Revert Cancelled Invoice"
                              >
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5">
